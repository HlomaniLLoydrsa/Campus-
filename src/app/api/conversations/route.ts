...
export async function POST(request: Request) {
  ...
  if (type === 'direct') {
    ...
    const connected = db.prepare('SELECT * FROM connections WHERE userId = ? AND connectedUserId = ?').get(userId, otherUserId);
    if (!connected) return NextResponse.json({ error: 'You must be connected to message this person' }, { status: 403 });

    // Find existing direct conversation
    const all = db.prepare("SELECT * FROM conversations WHERE type = 'direct'").all() as any[];
    ...
    db.prepare('INSERT INTO conversations (id, type, participants) VALUES (?, ?, ?)').run(id, 'direct', JSON.stringify([userId, otherUserId]));
  ...
  const id = `conv_${crypto.randomUUID().slice(0, 8)}`;
  db.prepare('INSERT INTO conversations (id, type, name, participants) VALUES (?, ?, ?, ?)').run(id, 'group', name || 'Group Chat', JSON.stringify(parts));
  return NextResponse.json({ id, existing: false }, { status: 201 });
}
