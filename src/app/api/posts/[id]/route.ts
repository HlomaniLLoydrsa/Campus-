...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (action === 'like') {
    ...
    db.prepare('UPDATE posts SET likedBy = ?, likes = ? WHERE id = ?').run(JSON.stringify(newLikedBy), newLikedBy.length, id);
  ...
  if (action === 'save') {
    ...
    db.prepare('UPDATE posts SET savedBy = ? WHERE id = ?').run(JSON.stringify(newSavedBy), id);
    return NextResponse.json({ savedBy: newSavedBy });
  }

  if (action === 'share') {
    const shares = (post.shares || 0) + 1;
    db.prepare('UPDATE posts SET shares = ? WHERE id = ?').run(shares, id);
    return NextResponse.json({ shares });
  }

  if (action === 'comment' && content) {
    const commentId = `c_${Date.now()}`;
    db.prepare('INSERT INTO comments (id, postId, authorId, content, likes, likedBy, createdAt) VALUES (?, ?, ?, ?, 0, ?, ?)').run(commentId, id, userId, content, '[]', new Date().toISOString());
