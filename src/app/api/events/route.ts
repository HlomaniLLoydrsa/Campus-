...
export async function POST(request: Request) {
  ...
  if (action === 'join') {
    ...
    if (eventData.joinType === 'approval') {
      ...
      db.prepare('INSERT OR IGNORE INTO event_participants (postId, userId, status) VALUES (?, ?, ?)').run(postId, userId, 'pending');
      // Notify creator
      if (post.authorId) {
        const joiner = db.prepare('SELECT name FROM users WHERE id = ?').get(userId) as any;
        const nid = `n_${crypto.randomUUID().slice(0, 8)}`;
        db.prepare('INSERT INTO notifications (id, userId, type, fromUserId, message, read) VALUES (?, ?, ?, ?, ?, 0)').run(nid, post.authorId, 'event-join-request', userId, `${joiner?.name || 'Someone'} requested to join ${eventData.name}`);
      }
    } else {
      eventData.participants.push(userId);
      eventData.currentParticipants = eventData.participants.length;
      db.prepare('INSERT OR IGNORE INTO event_participants (postId, userId, status) VALUES (?, ?, ?)').run(postId, userId, 'joined');
    }
    ...
    eventData.currentParticipants = eventData.participants.length;
    db.prepare('DELETE FROM event_participants WHERE postId = ? AND userId = ?').run(postId, userId);
  }

  db.prepare('UPDATE posts SET eventData = ? WHERE id = ?').run(JSON.stringify(eventData), postId);
