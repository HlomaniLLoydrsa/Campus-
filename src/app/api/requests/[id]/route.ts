...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (action === 'cancel') {
    ...
    await db.prepare("UPDATE connection_requests SET status = 'cancelled' WHERE id = ?").run(id);
  ...
  if (action === 'accept') {
    ...
    await db.prepare('INSERT OR IGNORE INTO connections (userId, connectedUserId, type) VALUES (?, ?, ?)').run(req.fromUserId, req.toUserId, connType);
    await db.prepare('INSERT OR IGNORE INTO connections (userId, connectedUserId, type) VALUES (?, ?, ?)').run(req.toUserId, req.fromUserId, connType);

    // Update request status
    await db.prepare("UPDATE connection_requests SET status = 'accepted' WHERE id = ?").run(id);

    // Notify the sender
    const accepter = await db.prepare('SELECT name FROM users WHERE id = ?').get(userId) as any;
  ...
  if (action === 'reject') {
    ...
    await db.prepare("UPDATE connection_requests SET status = 'rejected' WHERE id = ?").run(id);
