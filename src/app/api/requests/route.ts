...
export async function POST(request: Request) {
  ...
  await db.prepare('INSERT INTO connection_requests (id, fromUserId, toUserId, type, status) VALUES (?, ?, ?, ?, ?)').run(id, fromUserId, toUserId, type, 'pending');

  // Get sender's name for notification
  const sender = await db.prepare('SELECT name FROM users WHERE id = ?').get(fromUserId) as any;
