...
export async function PATCH(request: Request) {
  ...
  if (action === 'reveal') {
    ...
    if (isSender) await db.prepare('UPDATE secret_admirers SET revealFrom = 1 WHERE id = ?').run(id);
    if (isRecipient) await db.prepare('UPDATE secret_admirers SET revealTo = 1 WHERE id = ?').run(id);

    const updated = await db.prepare('SELECT * FROM secret_admirers WHERE id = ?').get(id) as any;
    // Mutual consent → reveal identities to both
    if (updated.revealFrom && updated.revealTo) {
      await db.prepare("UPDATE secret_admirers SET status = 'revealed' WHERE id = ?").run(id);
      const sender = await db.prepare('SELECT name FROM users WHERE id = ?').get(sa.fromUserId) as any;
  ...
  // ignored / blocked
  await db.prepare('UPDATE secret_admirers SET status = ? WHERE id = ?').run(action, id);
