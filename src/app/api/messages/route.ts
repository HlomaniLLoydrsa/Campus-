...
export async function GET(request: Request) {
  ...
  const db = getDb();
  const allConversations = db.prepare('SELECT * FROM conversations').all() as any[];
  ...
  const result = userConversations.map(conv => {
    const messages = db.prepare('SELECT * FROM messages WHERE conversationId = ? ORDER BY createdAt ASC').all(conv.id) as any[];
...
export async function POST(request: Request) {
  ...
  if (conv.type === 'direct') {
    ...
    if (otherUser) {
      const connected = db.prepare('SELECT * FROM connections WHERE userId = ? AND connectedUserId = ?').get(senderId, otherUser);
      if (!connected) {
  ...
  db.prepare('INSERT INTO messages (id, conversationId, senderId, content, read, createdAt) VALUES (?, ?, ?, ?, 0, ?)').run(id, conversationId, senderId, content, createdAt);
  ...
  parts.filter(p => p !== senderId).forEach(uid => {
    ...
    db.prepare('INSERT INTO notifications (id, userId, type, fromUserId, message, read) VALUES (?, ?, ?, ?, ?, 0)').run(nid, uid, nType, senderId, msg);
  });
...
export async function PATCH(request: Request) {
  ...
  const db = getDb();
  db.prepare('UPDATE messages SET read = 1 WHERE conversationId = ? AND senderId != ?').run(conversationId, userId);
